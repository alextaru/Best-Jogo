...
public class HtmlFunc {
	private String arquivo, condicao1,erro;// string arquivo Ž onde Ž gravado o
										// arquivo organizado
	private int clear;
	...
	public String HtmlRead() {
		try {
			...
			while (file.ready()) {
				...
				if (inicio != -1) {// correcao do erro quando nao acha os
									// caracteres < e > retorna -1
					...
					if (palavra.equals(condicao1)) {
						...
						palavra = linha.substring(inicio + 1, fim);// pega a
																	// proxima
																	// palavra
																	// em os
																	// caracteres
																	// < e >
						if (cont == 0) {// cont para separar data de numeros dos
										// jogos
							try {

								// testa se a palavra achada entre os caracteres
								// Ž uma data
								java.util.Date dataJogo = null;
								dataJogo = (java.util.Date) formato
										.parse(palavra);
								if (clear == 0) {
									arquivo = palavra + " - ";
									clear++;
